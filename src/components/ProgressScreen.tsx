...
import WorkoutLog, { WorkoutEntry } from '@/components/WorkoutLog';
import ProgressTracker, { WeightEntry } from '@/components/ProgressTracker';
...
interface Props {
  ...
  onAddWeight: (entry: { date: string; weightKg: number; notes: string }) => Promise<void>;
...
function SubTabButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`tap-scale min-h-11 flex-1 rounded-md text-sm font-black ${
        active ? 'bg-brand text-white shadow-[0_10px_22px_rgba(11,107,88,0.18)]' : 'text-muted'
      }`}
    >
      {children}
    </button>
